...
module RubyAnalyzer
  class Itembase
    ...
    class << self
      ...
      def register( item )
        if item.instance_of?(Class) || item.instance_of?(Module)
        ...
        0.upto(size - 1).each do |index|
          ...
          ancestor_list.each do |ancestor_klass_index|
            ...
            INFO_STURCT_WITHOUT_FALSE_SYMBOLS.each do |kind|
              ...
              next unless ancestor_item.respond_to?(:iv)
      ...
      def sort_by_ns_key
        ...
        xhs2 = xhs.reject{ |k, _v| k.nil? }
        xhs3 = xhs2.sort_by{|k2, _v2| k2}
    ...
    def init_basic(obj)
      ...
      key_array = ancestor_list.map(&:to_s)
    ...
    def set_open_class_info( from )
      ...
      INFO_STRUCT_SYMBOLS.map do |sym|
        tmp = from.iv[sym]
        unless tmp
          tmp = []
          from.iv[sym] = tmp
    ...
    def add_noname_defined_constant( x )
      raise unless x.instance_of?( String )
